...
app.config['MONGO_URI'] = 'mongodb+srv://davidd9:Micey#12@cluster0-liqmz.gcp.mongodb.net/InvestmenTracker?retryWrites=true'
...
@app.route('/deletestockinfo', methods=['POST'])
def deletestocks():
...
@app.route('/retrievestockinfo', methods=['GET'])
def getstocks():
    if 'username' in session:
        username = session['username']
        userstockdb = mongo.db.UserStocks
...
@app.route('/updatestockinfo', methods=['POST'])
def updatestocks():
    username = session['username']
    stockname = request.json['Stock']
...
@app.route('/scrapingstockdata', methods=['POST'])
def stockscrape():
    try:
...
@app.route('/scrapinggurunews', methods=['GET'])
def gurunews():
